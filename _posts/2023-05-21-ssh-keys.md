...
**NOTE**: the default path is `~/.ssh/id_rsa`, if you already have a key there it will be deleted.

This will create a public key `~/.ssh/id_rsa.pub` and a private key `~/.ssh/id_rsa`. Do **not** share the private key with anyone. It is recommended you put a password on the key, this means that if someone steals the private key, they cannot use it without the password.
...
To make use of SSH keys, you must first given your **public key** to the server you are trying to authenticate with. There are 2 methods of adding keys to a server.
